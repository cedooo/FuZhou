...
/****************************************************** 初始化 ***********************************************************/
   ...
   var record_start = 0;
    ...
    /** combo下拉框-字段local初始化 */
    var conditionFiledStore = [['projectName','项目名称'],['projectNumber','项目编号'],['connactName','项目负责人'],
	...
	var gridPanelDataStore = new Ext.data.JsonStore({
    		...
    		fields:['projectId','projectName','projectNumber','connactName','connactNumber','delFlg','descp']
		});
 /*************************************************** Form集合 ***********************************************************/
     	/** form的提示信息位置 */
    	...
    	var formPanel_view = new Ext.FormPanel({
						        title: '项目-查看',labelWidth:100,frame:true,defaultType:'textfield',
						        defaults:{labelStyle:"text-align:right",anchor:'95%',readOnly:true},
						        items: [
						        		{id:"viewProjectName",fieldLabel:'<font color="red">(*)</font>项目名称'},
						        		{id:"viewProjectNumber",fieldLabel:'项目编号'},
						        		{id:"viewConnactName",fieldLabel:'项目负责人' }, 
    	...
    	var formPanel_add = new Ext.FormPanel({
						        title:'项目-新增',labelWidth:100,frame:true,defaultType:'textfield', 
						        url:'B_ProjectServiceImpl!add.zdy',
						        defaults:{labelStyle:"text-align:right;",anchor:'95%'},
						        items:[{
						        		name:'B_Project.projectName',fieldLabel:'<font color="red">(*)</font>项目名称',
    									emptyText:"请输入项目名称",allowBlank:false,blankText:"项目名称不能为空" 
						            },{
						                name:'B_Project.projectNumber',fieldLabel:'项目编号'
						            },{
						                name:'B_Project.connactName',fieldLabel:'项目负责人'
						            }, {
						            	name:'B_Project.connactNumber',xtype:'numberfield',fieldLabel:'负责人联系电话'
						            }, {
						            	name:'B_Project.delFlg',xtype:'combo',fieldLabel:'<font color="red">(*)</font>是否启用',
						            	...
						            	name:'B_Project.descp',fieldLabel:'备注',xtype:'textarea'
							        }
						        ]
						    });
    	...
    	var formPanel_update = new Ext.FormPanel({
						        title: '项目-更新',labelWidth:100,frame:true,defaultType: 'textfield',
						        url:'B_ProjectServiceImpl!update.zdy',
						        defaults:{labelStyle :"text-align:right;",anchor:'95%'},
						        items:[{	
						        		id:"updateProjectId",name:'B_Project.projectId',hidden:true
						            },{
						        		id:"updateProjectName",name:'B_Project.projectName',fieldLabel: '<font color="red">(*)</font>项目名称',
						                emptyText:"请输入项目名称",allowBlank:false,blankText:"项目名称不能为空" 
						            },{
						            	id:"updateProjectNumber",name:'B_Project.projectNumber',fieldLabel: '项目编号'
						            },{
						            	id:"updateConnactName",name:'B_Project.connactName',fieldLabel:'项目负责人'
						            }, {
						            	id:"updateConnactNumber",name:'B_Project.connactNumber',xtype:'numberfield',fieldLabel: '负责人联系电话'
						            },{
						            	id:"updateDelFlg",name:'B_Project.delFlg',xtype:'combo',fieldLabel:'<font color="red">(*)</font>是否启用',
						                emptyText:"请选择是否启用",allowBlank:false,blankText:"是否启用不能为空",  
						                mode: 'local',store:delFlgStore,
						                typeAhead: true,triggerAction: 'all',forceSelection:true
						            },{
						            	id:"updateDescp",name:'B_Project.descp',fieldLabel: '备注',xtype:'textarea'
							        }
						        ]
						    });
		...
		var formPanel_import = new Ext.FormPanel({
						        title: '项目-上传',labelWidth:100,defaultType:'textfield',frame:true,fileUpload :true,
						        url:'B_ProjectServiceImpl!importExcel.zdy',
						        items: [{
						        ...
						        buttons: [{
						            ...
						            handler:function(){
									   Ext.Ajax.request({
											url: 'B_ProjectServiceImpl!downloadExcel.zdy',
     ...
     /************************************************ 窗口集合 ************************************************ */					    
    	...
    	var window_view = new Ext.Window({
	         					title:'查看窗口',closable:true,closeAction:"hide",width:400,height:300,
	            				iconCls:'bgi_add',plain:true,layout:'fit',items:formPanel_view,
	            				buttonAlign:'center',
						        buttons: [{text: '关闭',handler:function(){window_view.hide();}}]
	       					});				        				    
    	/** 新增窗口 */
    	var window_add = new Ext.Window({
	         					title:'新增窗口',closable:true,closeAction:"hide",width:400,height:300,
	         					iconCls:'bgi_add',plain:true,layout:'fit',items:formPanel_add,
	            				buttonAlign:'center',
						        buttons: 
						        [{
						            text:'保存',
    	...
    	var window_update = new Ext.Window({
	         					title:'更新窗口',closable:true,closeAction:"hide",width:400,height:320,
	         					iconCls:'bgi_update',plain:true,layout:'fit',items: formPanel_update,
	            				buttonAlign:'center',
						        buttons: 
						        [{
						            text:'保存',
  ...
  /************************************ Grid ***********************************************************/ 
   ...
   var functionGridPanel =  new xg.GridPanel({
        renderTo:"grid-size",title:'下拉维护表>>项目', width:Ext.get("grid-size").getWidth(),height:Ext.get("grid-size").getHeight(),
        ...
        cm: new xg.ColumnModel([sm,
            ...
            {header:"项目名称",      width: 140, sortable:true,dataIndex:'projectName'},
            {header:"项目编号",      width: 120, sortable:true,dataIndex:'projectNumber'},
            {header:"项目负责人",    width: 120, sortable:true,dataIndex:'connactName'},
            ...
            items:['-',{
            	 ...
            	 handler:function(){
            	 	var flag = 0;
            	  	/** 查询要导出的相应记录 */
						Ext.Ajax.request({
							 url: 'B_ProjectServiceImpl!exportExcel.zdy',
							 params: {
            	 ...
            	 handler:function(){
            	 	var _record = functionGridPanel.getSelectionModel().getSelected();
           		 	if (_record){
           		 		...
           		 		Ext.getCmp("updateProjectId").setValue(jsonData.get("projectId"));
           		 		Ext.getCmp("updateProjectName").setValue(jsonData.get("projectName"));
						Ext.getCmp("updateProjectNumber").setValue(jsonData.get("projectNumber"));
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认启用', '确认要启用选择的数据吗？', function(btn){
								...
								for (var i = 0;i<len;i++){
									if (i == 0) {
										jsonData = jsonData +  m[i].get("projectId");
									} else {
										jsonData = jsonData + "," +  m[i].get("projectId");
									}
								}
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_ProjectServiceImpl!effective.zdy',
									   params: {"ids":jsonData},
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认停用', '确认要停用选择的数据吗？', function(btn){
								...
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_ProjectServiceImpl!invalid.zdy',
									   params: {"ids":jsonData},
									   success: function(response,options){
									   		 ajaxSuccess(response,options);
									   },
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认删除', '确认要删除选择的数据吗？', function(btn){
								...
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_ProjectServiceImpl!delete.zdy',
									   params: {"ids":jsonData},
									   success: function(response,options){
									   		ajaxSuccess(response,options);
									   },
   ...
   /*********************************************** 事件初始化*******************************************************/
	...
	var onrowdoubleclick = function(functionGridPanel, index, e){
         ...
         if (_record){
           	var jsonData = functionGridPanel.getSelectionModel().getSelections()[0];
			Ext.getCmp("viewProjectName").setValue(jsonData.get("projectName"));
			Ext.getCmp("viewProjectNumber").setValue(jsonData.get("projectNumber"));
