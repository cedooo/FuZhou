...
public class A_OnuServiceImpl extends Forward {
	...
	public void add(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			templateDao.add(request.getAttribute(Constant.A_ONU));
	...
	public void update(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			TemplateDao templateDao = new TemplateDaoImpl();
			templateDao.update(request.getAttribute(Constant.A_ONU));
	...
	public void query(HttpServletRequest request, HttpServletResponse response)
		...
		String sql=" select onu.*,project.projectName as projectName,site.siteName as siteName," +
		   ...
		   " left join a_site  site on(onu.siteId  = site.siteId)" +
		   " left join b_manufacturers manufacturers on(onu.manufacturersId  = manufacturers.manufacturersId)" +
		   " left join b_constructionUnit constructionUnit on(onu.constructionUnitId = constructionUnit.constructionUnitId) ";
		try {
			TemplateDao templateDao = new TemplateDaoImpl();
			Map<String, Object> map = templateDao.combinedQuery((ConditionDto) request
					.getAttribute(Constant.CONDITIONDTO), pageDto, Constant.A_ONUDTO,sql);
			/** 将数据封装成json格式 */
	...
	public void exportExcel(HttpServletRequest request,
		...
		try {
			TemplateDao templateDao = new TemplateDaoImpl();
			Map<String, Object> map = templateDao.combinedQuery((ConditionDto) request
					.getAttribute(Constant.CONDITIONDTO),Constant.A_ONUDTO,sql);
			/** 生成excel */
			SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd");
			String excelName="onu表-"+simpleDateFormat.format(new Date())+".xls";
			String excelPath = Constant.EXPORTEXCELDIR +excelName;
			WritableWorkbook book = Workbook
					.createWorkbook(new File(excelPath));
			WritableSheet sheet = book.createSheet("onu列表", 0);
			/** 头部样式 */
			...
			Label headLabel1 = new Label(1, 0, "onu名称", headFormat);
			...
			Label headLabel10 = new Label(10, 0, "OSPF进程号", headFormat);
			sheet.addCell(headLabel10);
			sheet.setColumnView(10, 30);
			Label headLabel11 = new Label(11, 0, "VPN实例名", headFormat);
			...
			/** 正文内容 */
			int rowCount = 1;
			for (A_OnuDto a_OnuDto : (List<A_OnuDto>)map.get(Constant.BEENLIST)) {
				Label label0 = new Label(0, rowCount, String.valueOf(rowCount), bodyNormalFormat);
				sheet.addCell(label0);
				Label label1 = new Label(1, rowCount,a_OnuDto.getOnuName(), bodyNormalFormat);
				sheet.addCell(label1);
				Label label2 = new Label(2, rowCount,a_OnuDto.getSiteName(), bodyNormalFormat);
				sheet.addCell(label2);
				Label label3 = new Label(3, rowCount,a_OnuDto.getProjectName(), bodyNormalFormat);
				sheet.addCell(label3);
				Label label4 = new Label(4, rowCount,a_OnuDto.getInstallationSite(), bodyNormalFormat);
				sheet.addCell(label4);
				Label label5 = new Label(5, rowCount,a_OnuDto.getManufacturersName(),bodyNormalFormat);
				sheet.addCell(label5);
				Label label6 = new Label(6, rowCount,a_OnuDto.getTypeSpecification(),bodyNormalFormat);
				sheet.addCell(label6);
				Label label7 = new Label(7, rowCount, a_OnuDto.getConstructionUnitName(),bodyNormalFormat);
				sheet.addCell(label7);
				Label label8 = new Label(8, rowCount, a_OnuDto.getRunTime(),bodyNormalFormat);
				sheet.addCell(label8);
				Label label9 = new Label(9, rowCount, a_OnuDto.getVlanId(),bodyNormalFormat);
				sheet.addCell(label9);
				Label label10 = new Label(10, rowCount, a_OnuDto.getOspfNumber(),bodyNormalFormat);
				sheet.addCell(label10);
				Label label11 = new Label(11, rowCount, a_OnuDto.getVpnNumber(),bodyNormalFormat);
				sheet.addCell(label11);
				Label label12 = new Label(12, rowCount, a_OnuDto.getDelFlg(),bodyNormalFormat);
				sheet.addCell(label12);
				Label label13 = new Label(13, rowCount, a_OnuDto.getDescp(),bodyNormalFormat);
            	...
            	/** 判断是否是表单元素(<input type="text" />单选，多选等) */ 
            	 if (!item.isFormField()) {
                    ...
                    /** 未选择上传文件 */ 
            		 ...
            		 String fileName="onu_"+String.valueOf(System.currentTimeMillis())+".xls";
                     ...
                     try{
	                     ...
	                     for(int i=1;i<rows;i++){
	                    	...
	                    	if(st.getCell(0,i).getContents()==null||"".equals(st.getCell(0,i).getContents())){
                     			printWriter.append(Method.getJsonFormat(Message.ERRORMESSAGE_IMPORT+"+'第"+exceptionIndex+"行的onu名称为空'"));
                     			return;
	                    	}
	                    	...
	                    	/** 实例化onu模型 */
	                    	A_Onu onu = new A_Onu();
	                     	onu.setOnuName(st.getCell(0,i).getContents());
	                     	/** 下拉框查询实例 */
	                    	...
	                    	conditionDtoForCombo.setOrderValue("asc");
	                     	...
	                     	onu.setSiteId(siteList.get(0).getSiteId());
	                     	 /** 获取所属项目 */
	                     	if(!"".equals(st.getCell(2,i).getContents().trim())){
			                     	...
			                     	onu.setProjectId(projectList.get(0).getProjectId());
	                     	}
	                     	onu.setInstallationSite(st.getCell(3,i).getContents());
	                     	/** 获取所属厂家 */
	                    	conditionDtoForCombo.setConditionFiled("manufacturersName");
	                     	...
	                     	onu.setManufacturersId(manufacturersList.get(0).getManufacturersId());
	                     	onu.setTypeSpecification(st.getCell(5,i).getContents());
	                     	/** 获取施工单位 */
	                    	conditionDtoForCombo.setConditionFiled("constructionUnitName");
	                     	...
	                     	onu.setConstructionUnitId(constructionUnitList.get(0).getConstructionUnitId());
	                     	onu.setRunTime(st.getCell(7,i).getContents());
	                     	onu.setVlanId(st.getCell(8,i).getContents());
	                     	onu.setOspfNumber(st.getCell(9,i).getContents());
	                     	onu.setVpnNumber(st.getCell(10,i).getContents());
	                     	onu.setDelFlg(st.getCell(11,i).getContents());
	                     	onu.setDescp(st.getCell(12,i).getContents());
	                     	
	                     	TemplateDao templateDaoForTranscationAdd = new TemplateDaoImpl();
	                     	templateDaoForTranscationAdd.transcationAdd(onu,dBManager);
	                     } 
	...
	public void downloadExcel(HttpServletRequest request,
		...
		try {
			...
			/** 表头内容+列宽 */
			Label headLabel0 = new Label(0, 0, "onu名称(必填)", requiredFormat);
			onuSheet.addCell(headLabel0);
			onuSheet.setColumnView(0, 30);
			...
			onuSheet.addCell(headLabel1);
			onuSheet.setColumnView(1, 30);
			...
			onuSheet.addCell(headLabel2);
			onuSheet.setColumnView(2, 30);
			Label headLabel3 = new Label(3, 0, "安装地址", normalFormat);
			onuSheet.addCell(headLabel3);
			onuSheet.setColumnView(3, 30);
			...
			onuSheet.addCell(headLabel4);
			onuSheet.setColumnView(4, 30);
			Label headLabel5 = new Label(5, 0, "型号规格", normalFormat);
			onuSheet.addCell(headLabel5);
			onuSheet.setColumnView(5, 30);
			...
			onuSheet.addCell(headLabel6);
			onuSheet.setColumnView(6, 30);
			Label headLabel7 = new Label(7, 0, "投运时间", normalFormat);
			onuSheet.addCell(headLabel7);
			onuSheet.setColumnView(7, 30);
			Label headLabel8 = new Label(8, 0, "vlanId", normalFormat);
			onuSheet.addCell(headLabel8);
			onuSheet.setColumnView(8, 30);
			Label headLabel9 = new Label(9, 0, "ospf进程号", normalFormat);
			onuSheet.addCell(headLabel9);
			onuSheet.setColumnView(9, 30);
			Label headLabel10 = new Label(10, 0, "vpn实例名", normalFormat);
			onuSheet.addCell(headLabel10);
			onuSheet.setColumnView(10, 30);
			...
			onuSheet.addCell(headLabel11);
			onuSheet.setColumnView(11, 30);
			Label headLabel12 = new Label(12, 0, "备注", normalFormat);
			onuSheet.addCell(headLabel12);
			onuSheet.setColumnView(12, 30);
			/** 实例化查询接口 */
			TemplateDao templateDao = new TemplateDaoImpl();
			/** 站点集合 */
			{
