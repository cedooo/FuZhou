Ext.onReady(function(){
    ...
    var record_start = 0;
    /** combo下拉框-字段local初始化 */
    var conditionFiledStore = [['areaName','区域名称'],['connactName','区域负责人'],
	...
	var gridPanelDataStore = new Ext.data.JsonStore({
    		url: 'B_AreaServiceImpl!query.zdy',
    		baseParams:{start:0,limit:50},
    		root:'beenList',
    		totalProperty:'totalCount',
    		fields:['areaId','areaName','connactName','connactNumber','delFlg','descp']
		});
    /********************************** Form集合 ***********************************************************/
     	 /** form的提示信息位置 */
    	...
    	var formPanel_view = new Ext.FormPanel({
						        title: '区域-查看',labelWidth:100,frame:true, 
						        defaultType:'textfield',
						        defaults:{labelStyle:"text-align:right;",anchor:'95%',readOnly:true},
						        items: [
						        		{id:"viewAreaName",fieldLabel:'<font color="red">(*)</font>区域名称'},
						        		{id:"viewConnactName",fieldLabel:'区域负责人'},
    	...
    	var formPanel_add = new Ext.FormPanel({
						        title: '区域-新增',labelWidth:100,frame:true, 
						        url:'B_AreaServiceImpl!add.zdy',
						        defaultType:'textfield',
						        defaults:{labelStyle:"text-align:right;",anchor:'95%'},
						        items: [
						        	{
						        		name:'B_Area.areaName',fieldLabel:'<font color="red">(*)</font>区域名称',
    									emptyText:"请输入区域名称",allowBlank:false,blankText:"区域名称不能为空" 
						            },{
						                name:'B_Area.connactName',fieldLabel:'区域负责人'
						            },{
						            	name:'B_Area.connactNumber',xtype:'numberfield',fieldLabel:'负责人联系电话'
						            }, {
						            	name:'B_Area.delFlg',xtype:'combo',fieldLabel:'<font color="red">(*)</font>是否启用',
						            	...
						            	name:'B_Area.descp',fieldLabel:'备注',xtype:'textarea'
							        }
						        ]
						    });
    	...
    	var formPanel_update = new Ext.FormPanel({
						        title: '区域-更新',labelWidth:100,frame:true, 
						        url:'B_AreaServiceImpl!update.zdy',
						        defaultType: 'textfield',
						        defaults:{labelStyle :"text-align:right;",anchor:'95%'},
						        items: [
						        	{	
						        		id:"updateAreaId",name:'B_Area.areaId',hidden:true
						            },{
						        		id:"updateAreaName",name:'B_Area.areaName',fieldLabel:'<font color="red">(*)</font>区域名称',
						                emptyText:"请输入区域名称",allowBlank:false,blankText:"区域名称不能为空" 
						            },{
						            	id:"updateConnactName",name:'B_Area.connactName',fieldLabel:'区域负责人'
						            },{
						            	id:"updateConnactNumber",name:'B_Area.connactNumber',
						            	xtype:'numberfield',fieldLabel: '负责人联系电话'
						            },{
						            	id:"updateDelFlg",name:'B_Area.delFlg',xtype:'combo',fieldLabel:'<font color="red">(*)</font>是否启用',
						            	emptyText:"请选择是否启用",allowBlank:false,blankText:"是否启用不能为空",  
						                mode:'local',store:delFlgStore,
						                typeAhead:true,triggerAction:'all',forceSelection:true
						            },{
						            	id:"updateDescp",name:'B_Area.descp',
		...
		var formPanel_import = new Ext.FormPanel({
						        title: '区域-上传',labelWidth:100,
						        frame:true,fileUpload :true,
						        url:'B_AreaServiceImpl!importExcel.zdy',
						        defaultType:'textfield',
						        items: [{
						        ...
						        buttons: [{
						            ...
						            handler:function(){
									   Ext.Ajax.request({
											url: 'B_AreaServiceImpl!downloadExcel.zdy',
  ...
  /************************************ Grid ***********************************************************/ 
   ...
   var functionGridPanel =  new xg.GridPanel({
        renderTo:"grid-size",title:'下拉维护表>>区域', width:Ext.get("grid-size").getWidth(),height:Ext.get("grid-size").getHeight(),
        frame:true,iconCls:'bgi_grid',
        store:gridPanelDataStore,cls:"text-align:right;",
        cm: new xg.ColumnModel([sm,
            ...
            {header:"区域名称",      width:140,sortable:true,dataIndex:'areaName'},
            {header:"区域负责人",    width:120,sortable:true,dataIndex:'connactName'},
            ...
            items:['-',{
            	 ...
            	 handler:function(){
            	 	var flag = 0;
            	  	/** 查询要导出的相应记录 */
						Ext.Ajax.request({
							 url: 'B_AreaServiceImpl!exportExcel.zdy',
							 params: {
            	 ...
            	 handler:function(){
            	 	var _record = functionGridPanel.getSelectionModel().getSelected();
           		 	if (_record){
           		 		...
           		 		Ext.getCmp("updateAreaId").setValue(jsonData.get("areaId"));
           		 		Ext.getCmp("updateAreaName").setValue(jsonData.get("areaName"));
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认启用', '确认要启用选择的数据吗？', function(btn){
								...
								for (var i = 0;i <len; i++){
									if (i == 0) {
										jsonData = jsonData +  m[i].get("areaId");
									} else {
										jsonData = jsonData + "," +  m[i].get("areaId");
									}
								}
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_AreaServiceImpl!effective.zdy',
									   params: {"ids":jsonData},
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认停用', '确认要停用选择的数据吗？', function(btn){
								...
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_AreaServiceImpl!invalid.zdy',
									   params: {"ids":jsonData},
									   success: function(response,options){
									   		 ajaxSuccess(response,options);
									   },
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认删除', '确认要删除选择的数据吗？', function(btn){
								...
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_AreaServiceImpl!delete.zdy',
									   params: {"ids":jsonData},
									   success: function(response,options){
									   		ajaxSuccess(response,options);
									   },
   ...
   /*********************************************** 事件初始化*******************************************************/
	...
	var onrowdoubleclick = function(functionGridPanel, index, e){
         ...
         if (_record){
           	var jsonData = functionGridPanel.getSelectionModel().getSelections()[0];
			Ext.getCmp("viewAreaName").setValue(jsonData.get("areaName"));
