...
public class B_ProjectServiceImpl extends Forward {
	...
	public void add(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			templateDao.add(request.getAttribute(Constant.B_PROJECT));
	...
	public void update(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			TemplateDao templateDao = new TemplateDaoImpl();
			templateDao.update(request.getAttribute(Constant.B_PROJECT));
	...
	public void query(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Map<String, Object> map = templateDao.query((ConditionDto) request
					.getAttribute(Constant.CONDITIONDTO), pageDto, Constant.B_PROJECT);
			/** 将数据封装成json格式 */
	...
	public void exportExcel(HttpServletRequest request,
		...
		try {
			...
			String excelName="项目表-"+simpleDateFormat.format(new Date())+".xls";
			String excelPath = Constant.EXPORTEXCELDIR +excelName;
			WritableWorkbook book = Workbook
					.createWorkbook(new File(excelPath));
			WritableSheet sheet = book.createSheet("项目列表", 0);
			/** 头部样式 */
			...
			Label headLabel1 = new Label(1, 0, "项目名称", headFormat);
			sheet.addCell(headLabel1);
			sheet.setColumnView(1, 30);
			Label headLabel2 = new Label(2, 0, "项目编号", headFormat);
			sheet.addCell(headLabel2);
			sheet.setColumnView(2, 30);
			Label headLabel3 = new Label(3, 0, "项目负责人", headFormat);
			sheet.addCell(headLabel3);
			sheet.setColumnView(3, 30);
			Label headLabel4 = new Label(4, 0, "负责人联系电话", headFormat);
			...
			for (B_Project project : (List<B_Project>)map.get(Constant.BEENLIST)) {
				...
				Label label1 = new Label(1, rowCount,project.getProjectName(), bodyNormalFormat);
				sheet.addCell(label1);
				Label label2 = new Label(2, rowCount, project.getProjectNumber(), bodyNormalFormat);
				sheet.addCell(label2);
				Label label3 = new Label(3, rowCount,project.getConnactName(), bodyNormalFormat);
				sheet.addCell(label3);
				Label label4 = new Label(4, rowCount,project.getConnactNumber(), bodyNormalFormat);
				sheet.addCell(label4);
				Label label5 = new Label(5, rowCount, project.getDelFlg(),bodyNormalFormat);
				sheet.addCell(label5);
				Label label6 = new Label(6, rowCount, project.getDescp(),
						bodyNormalFormat);
            	...
            	/** 判断是否是表单元素(<input type="text" />单选，多选等) */ 
            	 if (!item.isFormField()) {
                    ...
                    /** 未选择上传文件 */ 
            		 ...
            		 String fileName="项目_"+String.valueOf(System.currentTimeMillis())+".xls";
	                     ...
	                     for(int i=1;i<rows;i++){
	                    	 /** 表格行数据都为空则跳过 */
		                    	...
		                    	if(st.getCell(0,i).getContents()==null||"".equals(st.getCell(0,i).getContents())){
	                     			printWriter.append(Method.getJsonFormat(Message.ERRORMESSAGE_IMPORT+"+'第"+exceptionIndex+"行的项目名称为空'"));
	                     			return;
	                     		}
	                     		...
	                     		project.setProjectName(st.getCell(0,i).getContents());
	                     		project.setProjectNumber(st.getCell(1,i).getContents());
	                     		project.setConnactName(st.getCell(2,i).getContents());
	                     		project.setConnactNumber(st.getCell(3,i).getContents());
	                     		project.setDelFlg(st.getCell(4,i).getContents());
	                     		project.setDescp(st.getCell(5,i).getContents());
	                     		TemplateDao templateDao = new TemplateDaoImpl();
	                 			templateDao.transcationAdd(project,dBManager);
	                     } 
	...
	public void downloadExcel(HttpServletRequest request,
		...
		try {
			...
			/** 表头内容+列宽 */
			Label headLabel0 = new Label(0, 0, "项目名称(必填)", requiredFormat);
			sheet.addCell(headLabel0);
			sheet.setColumnView(0, 30);
			Label headLabel1 = new Label(1, 0, "项目编号", normalFormat);
			sheet.addCell(headLabel1);
			sheet.setColumnView(1, 30);
			Label headLabel2 = new Label(2, 0, "项目负责人", normalFormat);
			sheet.addCell(headLabel2);
			sheet.setColumnView(2, 30);
