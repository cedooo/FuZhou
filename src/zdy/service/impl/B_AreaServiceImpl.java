...
public class B_AreaServiceImpl extends Forward {
	...
	public void add(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			templateDao.add(request.getAttribute(Constant.B_AREA));
	...
	public void update(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			TemplateDao templateDao = new TemplateDaoImpl();
			templateDao.update(request.getAttribute(Constant.B_AREA));
	...
	public void query(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Map<String, Object> map = templateDao.query((ConditionDto) request
					.getAttribute(Constant.CONDITIONDTO), pageDto, Constant.B_AREA);
			/** 将数据封装成json格式 */
	...
	public void exportExcel(HttpServletRequest request,
		...
		try {
			...
			String excelName="区域表-"+simpleDateFormat.format(new Date())+".xls";
			...
			Label headLabel2 = new Label(2, 0, "区域负责人", headFormat);
			sheet.addCell(headLabel2);
			sheet.setColumnView(2, 30);
			Label headLabel3 = new Label(3, 0, "负责人联系方式", headFormat);
			sheet.addCell(headLabel3);
			sheet.setColumnView(3, 30);
			Label headLabel4 = new Label(4, 0, "是否启用", headFormat);
			sheet.addCell(headLabel4);
			sheet.setColumnView(4, 30);
			Label headLabel5 = new Label(5, 0, "备注", headFormat);
			sheet.addCell(headLabel5);
			sheet.setColumnView(5, 30);
			/** 正文内容 */
			int rowCount = 1;
			for (B_Area b_Area : (List<B_Area>)map.get(Constant.BEENLIST)) {
				Label label0 = new Label(0, rowCount, String.valueOf(rowCount), bodyNormalFormat);
				sheet.addCell(label0);
				Label label1 = new Label(1, rowCount,b_Area.getAreaName(), bodyNormalFormat);
				sheet.addCell(label1);
				Label label2 = new Label(2, rowCount, b_Area.getConnactName(), bodyNormalFormat);
				sheet.addCell(label2);
				Label label3 = new Label(3, rowCount,b_Area.getConnactNumber(), bodyNormalFormat);
				sheet.addCell(label3);
				Label label4 = new Label(4, rowCount,b_Area.getDelFlg(), bodyNormalFormat);
				sheet.addCell(label4);
				Label label5 = new Label(5, rowCount, b_Area.getDescp(),bodyNormalFormat);
            	...
            	/** 判断是否是表单元素(<input type="text" />单选，多选等) */ 
            	 if (!item.isFormField()) {
                    ...
                    /** 未选择上传文件 */ 
            		 ...
            		 String fileName="区域_"+String.valueOf(System.currentTimeMillis())+".xls";
	                     ...
	                     for(int i=1;i<rows;i++){
	                    	  /** 表格行数据都为空则跳过 */
		                    	if( "".equals(st.getCell(0,i).getContents())&&"".equals(st.getCell(1,i).getContents())
			                      ...
			                      &&"".equals(st.getCell(4,i).getContents())){
			                     			continue;
			                    }
		                    	exceptionIndex = i;
		                    	if(st.getCell(0,i).getContents()==null||"".equals(st.getCell(0,i).getContents())){
	                     			printWriter.append(Method.getJsonFormat(Message.ERRORMESSAGE_IMPORT+"+'第"+exceptionIndex+"行的区域名称为空'"));
	                     			return;
	                     		}
	                     		if(st.getCell(3,i).getContents()==null||"".equals(st.getCell(3,i).getContents())){
	                     			printWriter.append(Method.getJsonFormat(Message.ERRORMESSAGE_IMPORT+"+'第"+exceptionIndex+"行的是否启用为空'"));
	                     			return;
	                     		}
	                     		B_Area area = new B_Area();
	                     		area.setAreaName(st.getCell(0,i).getContents());
	                     		area.setConnactName(st.getCell(1,i).getContents());
	                     		area.setConnactNumber(st.getCell(2,i).getContents());
	                     		area.setDelFlg(st.getCell(3,i).getContents());
	                     		area.setDescp(st.getCell(4,i).getContents());
	                     		TemplateDao templateDao = new TemplateDaoImpl();
	                 			templateDao.transcationAdd(area,dBManager);
	                     } 
