Ext.onReady(function(){
    ...
    var record_start = 0;
    /** combo下拉框-字段local初始化 */
    var conditionFiledStore = [['constructionUnitName','施工方名称'],['connactName','施工方负责人'],['constructionUnitAddress','施工方地址'],
	...
	var gridPanelDataStore = new Ext.data.JsonStore({
    		url: 'B_ConstructionUnitServiceImpl!query.zdy',
    		baseParams:{start:0,limit:50},
    		root:'beenList',
    		totalProperty:'totalCount',
    		fields:['constructionUnitId','constructionUnitName','connactName','constructionUnitAddress','connactNumber','delFlg','descp']
		});
    /********************************** Form集合 ***********************************************************/
     	 /** form的提示信息位置 */
    	...
    	var formPanel_view = new Ext.FormPanel({
						        title:'施工单位-查看',labelWidth:100,frame:true, 
						        defaultType:'textfield',
						        defaults:{labelStyle:"text-align:right;",anchor:'95%',readOnly:true},
						        items: [
						        		{id:"viewConstructionUnitName",fieldLabel:'<font color="red">(*)</font>施工方名称'},
						        		{id:"viewConnactName",fieldLabel:'施工方名称'},
						        		{id:"viewConstructionUnitAddress",fieldLabel:'施工方地址' }, 
    	...
    	var formPanel_add = new Ext.FormPanel({
						        title: '施工单位-新增',labelWidth:100,frame:true, 
						        url:'B_ConstructionUnitServiceImpl!add.zdy',
						        defaultType:'textfield',
						        defaults:{labelStyle:"text-align:right;",anchor:'95%'},
						        items: [
						        	{
						        		name:'B_ConstructionUnit.constructionUnitName',fieldLabel:'<font color="red">(*)</font>施工方名称',
    									emptyText:"请输入施工方名称",allowBlank:false,blankText:"施工方名称不能为空" 
						            },{
						                name:'B_ConstructionUnit.connactName',fieldLabel:'施工方负责人'
						            },{
						                name:'B_ConstructionUnit.constructionUnitAddress',fieldLabel:'施工方地址'
						            }, {
						            	name:'B_ConstructionUnit.connactNumber',xtype:'numberfield',fieldLabel:'负责人联系电话'
						            }, {
    	...
    	var formPanel_update = new Ext.FormPanel({
						        title: '施工单位-更新',labelWidth:100,frame:true, 
						        url:'B_ConstructionUnitServiceImpl!update.zdy',
						        defaultType: 'textfield',
						        defaults:{labelStyle :"text-align:right;",anchor:'95%'},
						        items: [
						        	{	
						        		id:"updateConstructionUnitId",name:'B_ConstructionUnit.constructionUnitId',hidden:true
						            },{
						        		id:"updateConstructionUnitName",name:'B_ConstructionUnit.constructionUnitName',fieldLabel: '<font color="red">(*)</font>施工方名称',
						                emptyText:"请输入施工方名称",allowBlank:false,blankText:"施工方名称不能为空" 
						            },{
						            	id:"updateConnactName",name:'B_ConstructionUnit.connactName',fieldLabel: '施工方负责人'
						            },{
						            	id:"updateConstructionUnitAddress",name:'B_ConstructionUnit.constructionUnitAddress',fieldLabel:'施工方地址'
						            }, {
						            	id:"updateConnactNumber",name:'B_ConstructionUnit.connactNumber',
		...
		var formPanel_import = new Ext.FormPanel({
						        title: '施工单位-上传',labelWidth:100,
						        frame:true,fileUpload :true,
						        url:'B_ConstructionUnitServiceImpl!importExcel.zdy',
						        defaultType:'textfield',
						        items: [{
						        ...
						        buttons: [{
						            ...
						            handler:function(){
									   Ext.Ajax.request({
											url: 'B_ConstructionUnitServiceImpl!downloadExcel.zdy',
  ...
  /************************************ Grid ***********************************************************/ 
   ...
   var functionGridPanel =  new xg.GridPanel({
        renderTo:"grid-size",title:'下拉维护表>>施工单位', width:Ext.get("grid-size").getWidth(),height:Ext.get("grid-size").getHeight(),
        ...
        cm: new xg.ColumnModel([sm,
            ...
            {header:"施工方名称",     width: 140, sortable:true,dataIndex:'constructionUnitName'},
            {header:"施工方负责人",   width: 120, sortable:true,dataIndex:'connactName'},
            {header:"施工方地址",    width: 120, sortable:true,dataIndex:'constructionUnitAddress'},
            ...
            items:['-',{
            	 ...
            	 handler:function(){
            	 	var flag = 0;
            	  	/** 查询要导出的相应记录 */
						Ext.Ajax.request({
							 url: 'B_ConstructionUnitServiceImpl!exportExcel.zdy',
							 params: {
            	 ...
            	 handler:function(){
            	 	var _record = functionGridPanel.getSelectionModel().getSelected();
           		 	if (_record){
           		 		...
           		 		Ext.getCmp("updateConstructionUnitId").setValue(jsonData.get("constructionUnitId"));
           		 		Ext.getCmp("updateConstructionUnitName").setValue(jsonData.get("constructionUnitName"));
						Ext.getCmp("updateConnactName").setValue(jsonData.get("connactName"));
						Ext.getCmp("updateConstructionUnitAddress").setValue(jsonData.get("constructionUnitAddress"));
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认启用', '确认要启用选择的数据吗？', function(btn){
								...
								for (var i = 0;i <len; i++){
									if (i == 0) {
										jsonData = jsonData +  m[i].get("constructionUnitId");
									} else {
										jsonData = jsonData + "," +  m[i].get("constructionUnitId");
									}
								}
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_ConstructionUnitServiceImpl!effective.zdy',
									   params: {"ids":jsonData},
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认停用', '确认要停用选择的数据吗？', function(btn){
								...
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_ConstructionUnitServiceImpl!invalid.zdy',
									   params: {"ids":jsonData},
									   success: function(response,options){
									   		 ajaxSuccess(response,options);
									   },
           		 ...
           		 handler:function(){
           		 	...
           		 	if (_record){
           		 		Ext.MessageBox.confirm('确认删除', '确认要删除选择的数据吗？', function(btn){
								...
								/** 更新相应记录 */
								Ext.Ajax.request({
									   url: 'B_ConstructionUnitServiceImpl!delete.zdy',
									   params: {"ids":jsonData},
									   success: function(response,options){
									   		ajaxSuccess(response,options);
									   },
   ...
   /*********************************************** 事件初始化*******************************************************/
	...
	var onrowdoubleclick = function(functionGridPanel, index, e){
         ...
         if (_record){
           	var jsonData = functionGridPanel.getSelectionModel().getSelections()[0];
			Ext.getCmp("viewConstructionUnitName").setValue(jsonData.get("constructionUnitName"));
			Ext.getCmp("viewConnactName").setValue(jsonData.get("connactName"));
			Ext.getCmp("viewConstructionUnitAddress").setValue(jsonData.get("constructionUnitAddress"));
